...
class PipelineRunner {
    ...
    RunDesignerPipeline(webApi) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            // create release
            core.info("1");
             let deployment = yield releaseApi.getDeployments(projectName, 14326, 54257);
             logger_1.Logger.LogPipelineTriggerOutput(deployment);
            core.info("2");
             deployment = yield releaseApi.getDeployments(projectName, 162, 14326);
             logger_1.Logger.LogPipelineTriggerOutput(deployment);
                        core.info("3");
             deployment = yield releaseApi.getDeployments(projectName, 162);
             logger_1.Logger.LogPipelineTriggerOutput(deployment);
                                    core.info("4");
             deployment = yield releaseApi.getDeployments(projectName, 162, 324);
             logger_1.Logger.LogPipelineTriggerOutput(deployment);
            if(deployment == null)
            {
