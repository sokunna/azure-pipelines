    ...
    RunDesignerPipeline(webApi) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (release != null) {
                ...
                setTimeout(() => __awaiter(this, void 0, void 0, function* () { return yield this.waitForPipeline(projectName, releaseApi, release); }), this.taskParameters.waitPeriod);
            }
     ...
     waitForRelease(projectName, releaseApi, release) {
        return __awaiter(this, void 0, void 0, function* () {
             core.debug("getting release result");
             let deployments = yield releaseApi.getDeployments(projectName, release.releaseDefinition.id, release.definitionEnvironmentId, release.createdBy.uniqueName, release.createdOn);
             let deployment = deployments[0];
            
            if (deployment.status == Release.Interfaces_1.DeploymentStatus.Succeeded || deployment.status == ReleaseInterfaces_1.DeploymentStatus.InProgress ) {
                logger_1.Logger.LogInfo(`Release is not yet competed, waiting... (status: ${build.status})`);
            ...
            logger_1.Logger.LogInfo(`Release is completed, result is: ${deployment.status}`);
            if (deployment.status !== Release.Interfaces_1.DeploymentStatus.Succeeded ) {
                let suffix = '';
                ...
                core.setFailed(`Release result is not Succeeded (2), instead: ${deployment.result}${suffix}`);
            }
        });
    }
}
