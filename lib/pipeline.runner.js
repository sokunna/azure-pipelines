...
class PipelineRunner {
    ...
    RunYamlPipeline(webApi) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            // if it's unset, let it be configured by input parameter
            if (sourceBranch === null && this.taskParameters.azurePipelineBranch) {
            ...
            if (buildQueueResult != null) {
                ...
                if (buildQueueResult.validationResults != null && buildQueueResult.validationResults.length > 0) {
                    ...
                    setTimeout(() => __awaiter(this, void 0, void 0, function* () { return yield this.waitForPipeline(buildApi, build.project.id, buildQueueResult.id); }), this.taskParameters.waitPeriod);
                }
            }
            else {
    ...
    waitForPipeline(buildApi, projectId, buildId) {
        return __awaiter(this, void 0, void 0, function* () {
            let build = yield buildApi.getBuild(projectId, buildId, 'result,status');
            if (build.status !== BuildInterfaces_1.BuildStatus.Completed) {
                let suffix = '';
                switch (build.status) {
                    case BuildInterfaces_1.BuildStatus.InProgress:
                        suffix = ' = In Progress';
                        break;
                    case BuildInterfaces_1.BuildStatus.Cancelling:
                        suffix = ' = Cancelling';
                        break;
                    case BuildInterfaces_1.BuildStatus.NotStarted:
                        suffix = ' = Not Started';
                        break;
                    case BuildInterfaces_1.BuildStatus.Postponed:
                        suffix = ' = Postponed';
                }
                logger_1.Logger.LogInfo(`Pipeline is not yet competed, waiting... (status: ${build.status}${suffix})`);
                setTimeout(() => __awaiter(this, void 0, void 0, function* () { return yield this.waitForPipeline(buildApi, projectId, buildId); }), this.taskParameters.waitPeriod);
                return;
            }
            logger_1.Logger.LogInfo(`Pipeline is completed, build result is: ${build.result}`);
            if (build.result != BuildInterfaces_1.BuildResult.Succeeded) {
                let suffix = '';
                switch (build.result) {
                ...
                logger_1.Logger.LogInfo(`Pipeline result is not Succeeded`);
    ...
    RunDesignerPipeline(webApi) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (release != null) {
                ...
                core.info("getting updated release...");
                let releaseupdate = yield releaseApi.getRelease(projectName, release.id);
                logger_1.Logger.LogPipelineTriggerOutput(releaseupdate);
                
                //setTimeout(() => __awaiter(this, void 0, void 0, function* () { return yield this.waitForRelease(projectName, releaseApi, release); }), this.taskParameters.waitPeriod);
            }
            
        });
    }
     waitForRelease(projectName, releaseApi, release) {
        return __awaiter(this, void 0, void 0, function* () {
             core.debug("getting release deployments...");
            
            
             let deployments = yield releaseApi.getDeployments(projectName, release.releaseDefinition.id, undefined, release.createdBy.uniqueName, release.createdOn);
            core.info("deployment found: " + deployments.length);
           
            let deployment = deployments[0];
            core.debug("deployment:");
            logger_1.Logger.LogPipelineTriggerOutput(deployment);
            if (deployment.deploymentStatus == ReleaseInterfaces_1.DeploymentStatus.NotDeployed || deployment.deploymentStatus == ReleaseInterfaces_1.DeploymentStatus.InProgress ) {
                logger_1.Logger.LogInfo(`Release is not yet competed, waiting... (status: ${deployment.deploymentStatus})`);
                setTimeout(() => __awaiter(this, void 0, void 0, function* () { return yield this.waitForRelease(projectName, releaseApi, release); }), this.taskParameters.waitPeriod);
                return;
            }
            if (deployment.deploymentStatus !== ReleaseInterfaces_1.DeploymentStatus.Succeeded ) {
                let suffix = '';
                switch (deployment.deploymentStatus) {
                    case ReleaseInterfaces_1.DeploymentStatus.PartiallySucceeded:
                        suffix = ' = partially succeeded';
                        break;
                    case ReleaseInterfaces_1.DeploymentStatus.Failed :
                        suffix = ' = Failed';
                        break;
                }
                logger_1.Logger.LogInfo(`Release result is not Succeeded`);
                core.setFailed(`Release result is not Succeeded, instead: ${deployment.deploymentStatus}${suffix}`);
            }
