...
class PipelineRunner {
    ...
    RunDesignerPipeline(webApi) {
     ...
     waitForRelease(projectName, releaseApi, release) {
        return __awaiter(this, void 0, void 0, function* () {
            core.info("Getting release update");
            let releaseupdate = yield releaseApi.getRelease(projectName, release.id);
            for(let i = 0; i < releaseupdate.environments.length; i++){
                let stage = releaseupdate.environments[i];
                if (stage.status == ReleaseInterfaces_1.EnvironmentStatus.NotStarted || stage.status == ReleaseInterfaces_1.EnvironmentStatus.InProgress || stage.status == ReleaseInterfaces_1.EnvironmentStatus.Queued ) {
                    let suffix = '';
                    switch (stage.status) {
                        case ReleaseInterfaces_1.EnvironmentStatus.NotStarted:
                            suffix = ' = NotStarted';
                            break;
                        case ReleaseInterfaces_1.EnvironmentStatus.InProgress :
                            suffix = ' = InProgress';
                            break;
                        case ReleaseInterfaces_1.EnvironmentStatus.Queued :
                            suffix = ' = Queued';
                            break;
                    
                    logger_1.Logger.LogInfo(`Release is not yet completed, waiting... (status: ${stage.status}${suffix})`);
                    setTimeout(() => __awaiter(this, void 0, void 0, function* () { return yield this.waitForRelease(projectName, releaseApi, release); }), this.taskParameters.waitPeriod);
                    return;
                }
                if (stage.status !== ReleaseInterfaces_1.EnvironmentStatus.Succeeded ) {
                    let suffix = '';
                    switch (stage.status) {
                        case ReleaseInterfaces_1.EnvironmentStatus.PartiallySucceeded:
                            suffix = ' = partially succeeded';
                            break;
                        case ReleaseInterfaces_1.EnvironmentStatus.Rejected :
                            suffix = ' = Rejeccted';
                            break;
                        case ReleaseInterfaces_1.EnvironmentStatus.Cancelled :
                            suffix = ' = Cancelled';
                            break;
                  }
                  logger_1.Logger.LogInfo(`Release result is not Succeeded`);
                  core.setFailed(`Release result is not Succeeded, instead: ${stage.status}${suffix}`);
                }
            }
